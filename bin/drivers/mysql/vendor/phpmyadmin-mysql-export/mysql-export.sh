...
if [[ $IS_PMA_4 -eq 1 ]]; then
  post_params+="&maxsize="
  post_params+="&mediawiki_caption=something"
  post_params+="&mediawiki_headers=something"
  post_params+="&mediawiki_structure_or_data=structure_and_data"
else
...
if [[ $IS_PMA_4 -eq 1 ]]; then
  post_params+="&pdf_structure_or_data=structure_and_data"
  post_params+="&phparray_structure_or_data=data"
else
...
if [[ $IS_PMA_4 -eq 1 ]]; then
  post_params+="&sql_create_table=something"
  post_params+="&sql_create_trigger=something"
  post_params+="&sql_create_view=something"
else
  post_params+="&sql_create_table_statements=something"
  post_params+="&sql_if_not_exists=something" # not in PMA4
fi
if [[ $IS_PMA_4 -eq 1 ]]; then
  post_params+="&structure_or_data_forced=0"
...
if [[ $IS_PMA_4 -eq 1 ]]; then
  post_params+="&sql_hex_for_binary=something"
else
...
if [  -n "$DB_TABLES" ] ; then
	...
	if [[ $IS_PMA_4 -eq 1 ]]; then
	  post_params+="&xml_export_events=something"
	fi
...
if [ -n "$STDOUT" ] ; then
decho "	Exportcommand: curl $curlopts -g -S -D $TMP_FOLDER/curl.headers -b $TMP_FOLDER/cookies.txt -c $TMP_FOLDER/cookies.txt $apache_auth_params -d "$post_params" $REMOTE_HOST/$export_script"
...
decho " Exportcommand: curl $curlopts -g -S -O -D $TMP_FOLDER/curl.headers -b $TMP_FOLDER/cookies.txt -c $TMP_FOLDER/cookies.txt $apache_auth_params -d "$post_params" $REMOTE_HOST/$export_script"
